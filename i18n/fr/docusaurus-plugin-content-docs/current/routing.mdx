...
Dans BowPHP, le système de routage est intégré à l'instance de l'application et exposé dans vos fichiers de routes via la variable `$router`.
...
Une *closure* est une fonction anonyme qui peut être définie à la volée et utilisée dans le routage de BowPHP. Pour plus d'informations, consultez la [documentation sur les closures](http://php.net/manual/fr/class.closure.php).
:::
...
Ceci va permettre à BowPHP de comprendre que vous voulez contacter la route associée à la méthode `DELETE`.
:::
...
BowPHP permet de restreindre des routes à un domaine ou sous-domaine spécifique.
...
Pour les projets en PHP 8+, BowPHP permet de déclarer les routes
