...
Le système de packages de BowPHP permet d'étendre votre application en greffant des modules externes réutilisables. Cette architecture modulaire facilite la maintenance et le partage de fonctionnalités entre projets.
...
class EmailCheckerConfiguration extends Configuration
{
  ...
  public function create(Loader $config): void
  {
    ...
    // bind() met le service en cache après la première résolution — c'est
    // le « singleton » de Bow. Pour une nouvelle instance à chaque appel,
    // utilisez factory() à la place.
...
Les fichiers PHP placés dans le dossier `config/` sont automatiquement chargés par BowPHP. Accédez-y avec la notation pointée : `$config['email-checker.blocked_domains']`.
:::
...
- [Conteneur d'injection de dépendances](./container) - Comprendre le DIC de BowPHP
