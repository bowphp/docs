---
id: storage
title: "Stockage de fichiers dans BowPHP"
---
...
- [Manipulation des fichiers](#manipulation-des-fichiers)
  ...
  - [Vérifier si le paramètre est un fichier](#vérifier-si-le-paramètre-est-un-fichier)
  - [Vérifier si le paramètre est un dossier](#vérifier-si-le-paramètre-est-un-dossier)
  - [Obtenir le chemin absolu d'un fichier ou dossier](#obtenir-le-chemin-absolu-dun-fichier-ou-dossier)

## Introduction

BowPHP intègre un système de gestion de fichier permettant de manipuler des fichiers avec une grande simplicité.
...
Pour gérer votre système de fichier local avec BowPHP vous allez utiliser la méthode statique `mount`.
...
> Si la méthode `mount` est appelée sans paramètre, elle utilisera par défaut la valeur de la clé `mount`.
...
Pour utiliser un système de stockage externe, cela se fait via la méthode `service`. Cette méthode vous permet de sélectionner le type de stockage externe que vous voulez utiliser. Vous trouverez la configuration nécessaire des services dans la section `services` du fichier `config/resource.php`.
