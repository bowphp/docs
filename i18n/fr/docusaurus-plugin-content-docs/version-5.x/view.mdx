...
BowPHP implémente 2 moteurs de template et le template par défaut est **[Tintin](./tintin)**.
:::
...
Pour fixer un code de statut HTTP, utilisez plutôt
`response()->render('vue', $data, $code)` (voir la documentation des
[réponses HTTP](./http-response)). `View::parse()` ne prend pas de code.
:::
...
:::info Tintin
Tintin est un template PHP qui se veut très simple et extensible. Il peut être utilisable dans n'importe quel projet PHP. Par défaut, Tintin est le moteur de template utilisé par BowPHP.
:::
...
### Ajouter vos directives via la configuration

Pour enregistrer vos directives au démarrage de BowPHP, créez (ou
modifiez) un provider dans `app/Configurations/` qui étend
...
N'oubliez pas d'enregistrer ce provider dans `Kernel::configurations()` pour
qu'il soit exécuté au boot.
