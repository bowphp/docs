...
Le conteneur d'injection de dépendances (DIC) de BowPHP, appelé **Capsule**, est un outil puissant pour gérer les dépendances de votre application.
...
Le conteneur propose trois primitives, distinguées par leur cycle de vie :

| Méthode | Quand l'objet est-il créé ? | Mise en cache ? |
|---------|-----------------------------|------------------|
| `bind($key, $value)`     | À la **première** résolution | Oui — réutilisée ensuite |
| `instance($key, $obj)`   | Immédiatement (vous le passez) | Oui — l'instance fournie est toujours rendue |
| `factory($key, $value)`  | À **chaque** résolution | Non — nouvelle instance à chaque appel |
...
// Lier une interface à une implémentation : Bow utilisera la réflexion pour
// instancier PaymentService et résoudra ses dépendances de constructeur.
...
// Raccourci via le helper : si le 2e argument est non vide, app() délègue
// automatiquement à makeWith().
