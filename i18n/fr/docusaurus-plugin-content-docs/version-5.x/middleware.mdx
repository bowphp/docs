...
class IpMiddleware implements BaseMiddleware
{
   ...
   * @param  callable $next
   * @param  array    $args   Paramètres optionnels (voir « Paramètres » plus bas)
   */
...
:::tip Contrat `BaseMiddleware`
L'interface `Bow\Middleware\BaseMiddleware` formalise la signature. Vos
middlewares peuvent l'implémenter (recommandé) ou simplement définir une
méthode `process()` compatible.
:::
...
// Vous pouvez aussi passer directement un FQCN sans déclarer d'alias :
$app->get('/', 'HomeController::index')
    ->middleware(\App\Middlewares\IpMiddleware::class);

// Via la méthode route() :
$app->route([
  'path' => '/',
  'method' => 'GET',
...
Vous pouvez passer des paramètres à un middleware en les ajoutant après le nom,
séparés par un `:` (puis des virgules pour plusieurs valeurs) :
...
Les valeurs sont disponibles dans `$args` :
...
:::note Parsing
Le nom et les paramètres sont séparés par le premier `:` rencontré
(`explode(':', $middleware, 2)`). Les valeurs sont ensuite séparées par
virgules. Aucun cast n'est fait — vos paramètres arrivent toujours sous forme
de chaînes.
:::
