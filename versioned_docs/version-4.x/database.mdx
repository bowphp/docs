---
id: database
title: "Base de données dans BowPHP"
---
...
- [Connexion à plusieurs bases de données](#connexion-à-plusieur-base-de-donnée)
...
La configuration de la base donnée de votre application se localise dans le fichier `config/database.php`. Dans ce fichier, vous pouvez définir toutes les connexions de votre base de donnée et un exemple est défini pour tous les types de base de donnée supportés.
...
Lorsque vous utilisez plusieurs connexions, vous pouvez accéder à chaque connexion via la méthode statique `connection` sur la classe [Bow\Database\Database::class](https://bowphp.com/api/master/Bow/Database/Database.html). Le nom transmis à la méthode de connexion doit correspondre à l'une des points de connexions répertoriées dans votre fichier de configuration `config/database.php`:
...
Une fois la configuration changée, elle est directement appliquée sur la connexion des modèles. [Cliquez ici](./orm) pour plus d'information sur les modèles.
...
Pour exécuter une requête brute de type `SELECT` vous devrez utiliser la méthode `Database::select` ou le helper `select`. On considère notre table `pets` et que nous sommes bien connectés à la base de donnée.

Exécution d'une requête pour obtenir toutes les informations de la table `pets`:
...
Pour faire une jointure dans BowPHP c'est très simple en fait. Pour ce faire on utilise la méthode `join`.
