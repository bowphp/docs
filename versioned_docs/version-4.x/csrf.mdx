---
id: csrf-protection
title: "Protection CSRF dans Bow Framework"
---
...
Le middleware `App\Middlewares\ClientCsrfMiddleware::class`, qui est inclus dans le groupe de middleware Web, vérifie automatiquement que le jeton dans l'entrée de la requête correspond au jeton stocké dans la session.
...
Bow stocke le jeton CSRF actuel dans un cookie `XSRF-TOKEN` inclus dans chaque réponse générée par le Framework. Vous pouvez utiliser la valeur de cookie pour définir l'en-tête de requête `X-XSRF-TOKEN`.
