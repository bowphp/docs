---
id: middleware
title: "Middlewares HTTP dans Bow Framework"
---
...
Différents Frameworks utilisent le système de middleware différemment. Bow ajoute un middleware sous forme de file d'attente au dessus de votre application principale. Chaque nouvelle couche de middleware est ajouté la file de middleware existante. La structure de file se dilate vers l'extérieur au fur et à mesure que les couches intermédiaires supplémentaires sont ajoutées.
...
Il est aussi possible d'ajouter les middlewares de façon globale dans l'application. Cela se fait dans le fichier de `routes` avec la méthode `middleware` sur `$app` qui va nous retourner une instance de `\Bow\Router\Router::class` donc toutes les routes qui seront définies avec cette instance hériteront des middlewares spécifiés.
