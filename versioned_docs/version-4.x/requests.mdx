...
- [Récupération des données](#récupération-des-données)
  ...
  - [Via un contrôleur](#via-un-contrôleur)
...
BowPHP dans son démarrage optimise le donnée envoyés au serveur en remplaçant les valeur vide par `null` et supprimer les spaces blancs dans les valeurs de chaque champs avec la fonction php `trim`.
...
> Pour plus d'information sur la session visitez cette page [HTTP Session](./seeding)

### Récupérer l'authentification courante

Vous pouvez aussi avoir l'authenticiation en cours si vous avez utiliser le système d'authentification native de BowPHP.
