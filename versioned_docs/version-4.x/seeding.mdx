---
id: seeding
title: "Préremplissage de la base de données dans Bow Framework"
---
...
Le seeding est un mécanisme qui vous permettra d'hydrater votre base de données pour pouvoir l'utiliser dans votre développement. Le seeding dans Bow est associé à une table lors de la création.
...
Vous avez la possibilité de lancer tous les seeders avec la commande `seed:all`. Cela fera le seeding de tous les fichiers dans `seeders`.
