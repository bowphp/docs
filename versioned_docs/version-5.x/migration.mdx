...
Les migrations sont sauvegardées dans le dossier `migration`.

## Ajouter une migration

Pour ajouter une migration, il faut passer par `php bow` avec la commande `add:migration` ensuite le nom de la migration (ex: `createTodosTable`). Bow créera un fichier du même nom précédé d'une date de création.
...
:::tip Structure d'une migration
Voici un exemple de la migration créée précédemment :
:::

Par exemple, la migration créée précédemment :
...
:::tip Raccourci
Vous pouvez aussi utiliser le raccourci `migrate`.
:::
...
Pour créer une nouvelle table de base de données, utilisez la méthode `create`. La méthode `create` accepte deux arguments. Le premier est le nom de la table, tandis que le second est une clôture qui reçoit un objet `\Bow\Database\Migration\Table::class` pouvant être utilisé pour définir la nouvelle table:
...
:::info Connexion personnalisée
Si vous souhaitez effectuer une opération de schéma sur une connexion à une base de données autre que la connexion par défaut, utilisez la méthode `connection`.
:::

```php
$this->connection('name')->create("users", function (Table $table) {
  $table->addIncrement('id');
  $table->addTimestamps();
});
```
...
> Avant de renommer une table, vous devez vérifier que toutes les contraintes de clé étrangère sur la table ont un nom explicite dans vos fichiers de migration au lieu de laisser BowPHP attribuer un nom basé sur une convention. Sinon, le nom de la contrainte de clé étrangère fera référence à l'ancien nom de la table.

## Créer des colonnes

La méthode de `alter` sur `Bow\Database\Migration\Migration::class` peut être utilisée pour mettre à jour des tables existantes. Comme la méthode `create`, la méthode `alter` accepte deux arguments: le nom de la table et une clôture qui reçoit une instance de `\Bow\Database\Migration\Table::class` que vous pouvez utiliser pour ajouter des colonnes à la table:
...
Notons pour l'instant que tous ces méthodes sont en réalité des helpers de la méthode `addColumn` de `\Bow\Database\Migration\Table::class`.
...
| primary  | **Boolean** | Définit la colonne comme une clé primaire. Ajout PRIMARY AUTO sur la description de la colonne |
| index  | **Boolean** | Définit la colonne comme une INDEX |
| increment  | **Boolean** | Définit la colonne comme une AUTO INCREMENT |
| size  | **Integer** | Définit la taille de la colonne |
| nullable  | **Boolean** | La colonne pourra avoir une valeur nulle |
| unsigned  | **Boolean** | Seulement pour les colonnes de type nombre. Elle définit la colonne comme non signée |
| default  | **Mixed** | Ajoute une valeur par défaut à la colonne si une valeur n'est pas ajoutée |
