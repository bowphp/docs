...
Souvent, vous sera tenté de faire des seeding lorsque vous testez votre application. Le helper `app_db_seed`, très pratique vous permettra de faire des seeding en plein test par exemple

```php
app_db_seed("user", $overrides = ["name" => "Franck"]);
```
