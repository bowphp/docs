...
The BowPHP scheduler offers a simple and elegant way to define scheduled tasks. It lets you automate recurring tasks such as database backups, sending emails, clearing caches, and much more.

:::info About the Scheduler
The scheduler uses cron expressions under the hood, but lets you define them with a fluent PHP API that is more readable and maintainable.
:::

## Main methods

The scheduler provides four main methods for scheduling different types of tasks:

| **Method** | **Description** |
|-------------|-----------------|
| `command()` | Run Bow console commands |
| `task()` | Run QueueTask classes |
| `exec()` | Run shell/bash commands |
| `call()` | Run closures/callbacks |

## Defining tasks

Scheduled tasks are defined in the `schedules()` method of your `App\Kernel` class. This approach offers better integration with the framework and a centralized configuration:
...
The `schedules()` method is automatically called when running the scheduler commands. All of your tasks are defined in the same place as your other application configuration.
:::
...
The scheduler offers many methods for defining the execution frequency:
...
To set the time of a recurring execution, use the `*At` variant
...
$schedule->command('report:generate')->weeklyOn(1, '08:00');   // Monday 8am
$schedule->command('report:generate')->monthlyOn(15, '15:00'); // the 15th at 3pm
$schedule->command('report:generate')->yearlyOn(6, 1, '17:00'); // June 1st 5pm
```
...
$schedule->command('custom:task')
  ->cron('0 */4 * * 1-5');  // Every 4 hours, Monday through Friday
```

## Day constraints

You can restrict execution to certain days:
...
Set a specific timezone for the scheduling:
...
Displays when each task will next be executed:
...
class Kernel extends ApplicationLoader
{
  ...
  // Check the system status every 5 minutes
  $schedule->call(function () {
