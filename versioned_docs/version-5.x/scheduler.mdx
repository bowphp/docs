...
Le planificateur de BowPHP offre une manière simple et élégante de définir des tâches planifiées. Il vous permet d'automatiser des tâches récurrentes comme les sauvegardes de bases de données, l'envoi d'emails, le nettoyage de cache, et bien plus encore.
...
class Kernel extends ApplicationLoader
{
  ...
  public function schedules(Scheduler $schedule): void
  {
    ...
    // Planifier un QueueTask le dimanche à 10h00
    // (1 = lundi … 7 = dimanche)
...
Pour fixer l'heure d'une exécution récurrente, utilisez la variante `*At`
correspondante à la fréquence — il n'existe **pas** de méthode `at()`
chaînable autonome.

```php
$schedule->command('report:generate')->dailyAt('09:00');
$schedule->command('report:generate')->hourlyAt(15);          // chaque heure à hh:15
$schedule->command('report:generate')->weeklyOn(1, '08:00');   // lundi 8h
$schedule->command('report:generate')->monthlyOn(15, '15:00'); // le 15 à 15h
$schedule->command('report:generate')->yearlyOn(6, 1, '17:00'); // 1er juin 17h
```
...
Exécute manuellement une tâche planifiée par le nom complet de sa classe :
...
class Kernel extends ApplicationLoader
{
  public function schedules(Scheduler $schedule): void
  {
  // Sauvegarde quotidienne de la base de données (1h00)
  $schedule->exec('mysqldump mydb > /backups/daily.sql')
    ->dailyAt('01:00')
    ->description('Sauvegarde quotidienne de la base de données')
    ->runInBackground();

  // Vider le cache chaque dimanche à 2h00 (1 = lundi, 7 = dimanche)
  $schedule->command('cache:clear')
    ->weeklyOn(7, '02:00')
  ...
  // Envoyer les rapports hebdomadaires le vendredi 17h00 (seulement en production)
  $schedule->task(\App\Tasks\SendWeeklyReportTask::class)
    ->weeklyOn(5, '17:00')
    ->timezone('Europe/Paris')
    ->when(function () {
