...
For example, you can write a middleware to check whether a user is logged in and then take an action accordingly. You can run code before and after your Bow application to manipulate the `Request` and `Response` objects however you like.
...
The only strict requirement is that a middleware must return a value other than `null`. Each middleware will need to call the next middleware and pass it the `Request` objects as arguments via the `$next` method.
...
All middlewares are saved by default in the `app/Middlewares` folder.
:::

The middleware we just added will check the client's IP address, and if its address equals `127.0.0.1`, we will say that the application is in development mode.

But first, let's look at the content of the `IpMiddleware` file. It is the `process` method that runs the middleware, and the callable is what runs the next middleware.
...
You can pass parameters to a middleware by adding them after the name,
...
:::note Parsing
The name and parameters are separated by the first `:` encountered
