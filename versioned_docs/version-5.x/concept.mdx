...
Le routeur est un composant clé de Bow Framework. Il agit comme une "boussole" qui oriente chaque requête vers le bon contrôleur et la bonne méthode.
...
:::tip
Ici :
- La requête `GET /users/42` est associée à la méthode `show` du `UserController`.
- La requête `POST /api/login` est associée à la méthode `login` du `AuthController`.
:::

Bow supporte aussi le **routage par attributs PHP 8**, qui permet de déclarer
les routes directement sur la classe contrôleur sans fichier de routes
centralisé :
...
Voir [Routage > Routage par attributs](./routing) pour le détail des attributs
disponibles.
...
:::tip
Bow Framework repose sur une architecture modulaire. Chaque module est
indépendant, possède sa propre configuration et remplit un rôle spécifique.
Cela rend l'application facile à gérer et à étendre. Le routeur en est un
exemple ; le module Database, qui gère les connexions à la base de données,
en est un autre.
:::

#### Le contrat : `Bow\Configuration\Configuration`

Tous les modules suivent le **même contrat** : chacun expose une classe qui
étend `Bow\Configuration\Configuration` et implémente deux méthodes :

- `create(Loader $config)` — déclare les liaisons dans le container
- `run()` — démarre le module (utile pour les ressources eager)
...
Prenons le cas du module **Database**, qui gère les connexions à la base de
données :
...
connexion est alors prête à être utilisée par les contrôleurs, les modèles
