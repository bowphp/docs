...
BowPHP est un micro-framework PHP moderne et élégant. Avant de créer votre première application, assurez-vous que votre environnement respecte les prérequis suivants.
...
BowPHP utilise [Composer](https://getcomposer.org) pour gérer ses dépendances. Assurez-vous que Composer est installé sur votre machine avant de continuer.
...
Tous les fichiers de configuration de BowPHP se trouvent dans le dossier `config`. Chaque option est documentée pour faciliter la personnalisation de votre application.
...
Pour déployer BowPHP sur un hébergement mutualisé :
