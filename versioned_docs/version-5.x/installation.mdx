...
If you use [Laragon](https://laragon.org/) (Windows), [XAMPP](https://www.apachefriends.org/), or [MAMP](https://www.mamp.info/), these extensions are generally already installed and configured.
:::
...
We recommend installing Composer globally on your machine. See the [official documentation](https://getcomposer.org/download) for installation instructions.

If you are new to Composer, take the time to read through its [documentation](https://getcomposer.org/doc/) to understand how it works.
:::
...
:::info Default port
If you do not specify a port, the server starts on port `5000` by default:
...
After installation, configure your web server's `document root` to point to the `public` directory.

The `index.php` file located in this directory serves as the single entry point for all HTTP requests (the **Front Controller** pattern).

### Configuration files

All of BowPHP's configuration files are located in the `config` directory. Each option is documented to make it easier to customize your application.
...
The directories contained in `var` must have write permissions for the web server:
...
:::warning Important
Make sure the web server has write permissions on the `var` directory so that logs, cache, and sessions work correctly.
:::
...
If the default `.htaccess` file does not work, try this alternative configuration:
...
To deploy BowPHP to shared hosting:

1. Move the `index.php` and `.htaccess` files from the `public` directory to the root of the project
2. Update the paths in `index.php` so that they correctly point to the framework's directories
