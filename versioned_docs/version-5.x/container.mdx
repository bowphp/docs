...
**Dependency injection** lets you provide the objects a class needs instead of having it create them itself. This makes the code more modular and testable.
...
| `instance($key, $obj)`   | Immediately (you pass it) | Yes — the provided instance is always returned |
| `factory($key, $value)`  | On **every** resolution | No — a new instance on each call |

### Simple binding (bind)

Registers a service that will be instantiated only once, then cached:
...
// Bind an interface to an implementation: Bow will use reflection to
// instantiate PaymentService and resolve its constructor dependencies.
...
:::info Benefits
Thanks to these principles, you can build cleaner applications that are easier to maintain and more robust.
:::
