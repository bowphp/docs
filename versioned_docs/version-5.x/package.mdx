...
In the configuration file, the [dependency injection container](./container) is accessible through `$this->container`.
...
:::info Automatic configuration loading
...
The `EmailCheckerController` controller automatically receives the `EmailCheckerService` service thanks to [dependency injection](./container). The container resolves constructor dependencies automatically.
:::
...
- [Dependency injection container](./container) - Understand BowPHP's DIC
- [Services](./service) - Create and use services
- [Configuration](./configuration) - Manage configuration files
- [Routing](./routing) - Define routes in your application
