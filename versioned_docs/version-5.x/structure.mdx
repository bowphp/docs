...
BowPHP follows the **MVC** (Model-View-Controller) pattern, a proven architecture that clearly separates your application's responsibilities.
...
Event system to decouple your application. See the [events documentation](./messaging).
...
Contains all the application's configuration files.
...
:::warning Security
Configure your web server to point to this directory only. Never expose the other directories.
:::
...
| `session/` | Session files (when using the file driver) |
...
- **One file = one class**: Follow PSR-4 conventions.
