...
Le routing vous permet de mapper une URL sur un controlleur ou une action particulière.
...
La mise en place du routage se faire donc via les méthodes suivantes :

### Mapping avec `get`

Cette méthode permet de mapper une URL à une requête de type `GET`.
...
Cette méthode permet de mapper une URL à une requête de type `POST`
...
Cette méthode permet de mapper une URL à une requête de type `PUT`.
...
Cette méthode permet de mapper une URL à une requête de type `DELETE`.
...
Cette méthode permet de mapper une URL à une requête de type `PATCH`.
...
Cette méthode permet de mapper une URL à une requête de type `OPTIONS`.
...
Les navigateurs ne supportent pas cette méthode. Alors dans votre formulaire d'envoyer, il faudra créer des champs comme ceci :
...
Permet d'associer tout les méthodes `http` sur l'URL spécifié.
...
Souvent, vous serez amener à grouper vos routes et effectuer un branchement simple pour bien orienter votre logique. Les URLs peuvent souvent se répéter comme ceci :
...
Dans ce cas nous avons `users` qui se répété plusieurs fois. Comment bien organiser tout ça ? La réponse est la préfixale de route. Alors la méthodes `prefix` nous permet de grouper plusieur URLs.
...
> Notez qu'actuellement le préfix ne peut pas être imbriqué.
...
> Plus d'information sur les controllers [ici](./controllers)

- Avec le nom d'un controller et un middleware.
...
- Avec le nom d'un controller, un middleware et une restriction.
...
Ensuite la variable capturée sera passée en paramètre à l'action (fonction à execute dans le cas où l'url est valide)
Quel que soit le nombre de variables.
...
aussi d'ajouter des validateurs sur la variable. C'est la méthode `where` qui s'en occupe.
...
```php
$app->get('/:name', ['middleware' => 'ip', function (string $name)
{
  return 'Bonjour ' . $name;
}]);
