...
A *closure* is an anonymous function that can be defined on the fly and used in BowPHP routing. For more information, see the [closures documentation](http://php.net/manual/fr/class.closure.php).
:::
...
directly on controller classes using attributes. No need to
maintain a central route file.
...
Each method attribute is repeatable: the same method can serve
