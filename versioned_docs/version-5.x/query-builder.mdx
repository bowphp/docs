...
Bow provides an API for building queries. The `table` method lets you build an SQL query based on the table name and returns an instance of [`Bow\Database\QueryBuilder::class`](https://bowphp.com/api/master/Bow/Database/QueryBuilder.html).
...
For example, the `toSql` method, which lets you display the query you built.
...
To insert a record and retrieve the auto-incremented value of the new record
