---
id: mail
title: "Sending mail with BowPHP"
---

import SuggestionFeature from "@site/src/components/Partials/SuggestionFeature";

## Introduction

Sending emails is a common requirement in any application. BowPHP ships with an easy-to-use email sending system. You can use this system through several drivers:

- **smtp** — Direct sending via an SMTP server
- **ses** — Sending via [Amazon SES](https://aws.amazon.com/fr/ses/)
- **mail** — Uses PHP's native `mail()` function
- **log** — Writes the email to the logs (handy during development)

## Configuration

You will find the configuration in the `config/mail.php` file, which is relatively simple. All the options are commented.

:::info SES driver
Install the `aws/aws-sdk-php` package if you want to use `ses` as your mail driver.

```bash
composer require aws/aws-sdk-php
```
:::

### Switching drivers at runtime

```php
use Bow\Mail\Mail;

Mail::setDriver('log');     // route all sends to the log
Mail::setDriver('smtp');    // back to the configured driver
```

## Usage

To use the service, call the `Bow\Mail\Mail` class. It exposes the
following static methods:

| Method | Usage |
|---------|-------|
| `send($view, $data, $cb)` | Sends an email based on a view |
| `raw($to, $subject, $message, $headers = [])` | Sends raw text |
| `queue($view, $data, $cb)` | Queues the send |
| `queueOn($queue, $view, $data, $cb)` | Named queue |
| `later($delay, $view, $data, $cb)` | Delayed send (in seconds) |
| `laterOn($delay, $queue, $view, $data, $cb)` | Delayed send on a named queue |
| `setDriver($driver)` | Switch drivers on the fly |

### The `send` method

`send` lets you send emails based on a [view](./views).

#### Prototype

```php
Mail::send(string $view, array|callable $data, ?callable $cb = null): bool
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `$view`   | `string` | The name of the view to use |
| `$data`   | `array` or `callable` | Data passed to the view. If you omit it and pass the builder directly as the 2nd argument, that is valid too. |
| `$cb`     | `?callable` | Envelope builder (see `Envelop` below) |

The callback receives an instance of [`Bow\Mail\Envelop`](https://bowphp.com/api/master/Bow/Mail/Envelop.html), which lets you add the recipient, the subject, attachments, the sender, and so on.

#### Example

Consider the view `email-view.tintin.php`:

```php title="templates/email-view.tintin.php"
Bonjour {{ $name }},

Nous vous informons que votre compte vient d'être crédité de 100.000.000 F.

Cordialement,
```

The code that sends the mail:

```php
use Bow\Mail\Mail;
use Bow\Mail\Envelop;

$data = ['name' => 'Franck'];

Mail::send('email-view', $data, function (Envelop $envelop) {
  $envelop->to('info@exemple.com');
  $envelop->subject('Paiement !');
  $envelop->from('info@exemple.com', 'Bow');
});
```

Short form (without data for the view):

```php
Mail::send('email-view', function (Envelop $envelop) {
  $envelop->to('info@exemple.com')
          ->subject('Bonjour')
          ->from('info@exemple.com', 'Bow');
});
```

### The `Envelop` API

The `Bow\Mail\Envelop` object accumulates recipients, headers, and the body
of the message. All of its methods return `$this`, which allows chaining.

#### Recipients

```php
$envelop->to('info@exemple.com');               // one recipient
$envelop->to(['a@x.com', 'b@x.com']);            // several recipients
$envelop->addCc('cc@x.com', 'Nom CC');
$envelop->addBcc('bcc@x.com');
$envelop->addReplyTo('reply@x.com');
$envelop->addReturnPath('bounces@x.com');
```

:::note Aliases
The short methods `cc()`, `bcc()`, `replyTo()`, and `returnPath()` are
aliases of their `addCc`/`addBcc`/`addReplyTo`/`addReturnPath` counterparts and have
the same signature.
:::

#### Subject and sender

```php
$envelop->subject('Objet du message');
$envelop->from('noreply@x.com', 'Mon App');
```

#### Message body

```php
$envelop->html('<h1>Bonjour</h1>');   // HTML content (text/html type)
$envelop->text('Bonjour');             // plain text content
$envelop->view('emails.welcome', ['user' => $user]); // renders a view
```

#### Custom headers

```php
$envelop->withHeader('X-Custom-Header', 'valeur');
```

#### Attachments

```php
$envelop->addFile('/chemin/vers/fichier.pdf');
```

#### Priority

```php
// X-Priority priority: 1 (highest) to 5 (lowest)
$envelop->setPriority(1);     // or $envelop->addPriority(1)
```

### The `raw` method

`raw` lets you send a raw message (without a view):

```php
Mail::raw(string|array $to, string $subject, string $data, array $headers = []): mixed
```

| Parameter | Type | Description |
|-----------|------|-------------|
| `$to`       | `string\|array` | Recipient email(s) |
| `$subject`  | `string` | The subject of the mail |
| `$data`     | `string` | The message body |
| `$headers`  | `array`  | Additional headers |

```php
use Bow\Mail\Mail;

Mail::raw(
  'hello@exemple.com',
  'Hello, world',
  "C'est juste un email de test"
);
```

## Queued sending

To avoid blocking the HTTP request, queue the send.
The `(view, data, cb)` signature is identical to `send`.

```php
use Bow\Mail\Mail;
use Bow\Mail\Envelop;

// Default queue
Mail::queue('email-view', ['name' => 'Franck'], function (Envelop $e) {
  $e->to('info@exemple.com')->subject('Bonjour')->from('app@x.com');
});

// Named queue (high priority, for example)
Mail::queueOn('high', 'email-view', $data, $callback);

// Send delayed by 60 seconds
Mail::later(60, 'email-view', $data, $callback);

// Delayed on a named queue
Mail::laterOn(3600, 'mails', 'email-view', $data, $callback);
```

:::tip Worker required
Remember that a worker must be running (`php bow run:worker`) to process
queued jobs.
:::

<SuggestionFeature />
