...
For more information on HTTP codes, see this [link](https://en.wikipedia.org/wiki/List_of_HTTP_status_codes).
...
You will often need to set up systems for downloading zip files or images. You will also build applications that require authentication before downloading files (Dropbox-style). Bow offers a simple API via the [`Bow\Http\Response`](https://bowphp.com/api/master/Bow/Http/Response.html) class to download a file with the `download` method.
...
More information about the [`Content-Disposition`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Disposition) header.
:::
...
You will certainly need to redirect to other resources, and this is in fact an HTTP response. The `to` and `back` methods (the latter, as its name suggests, lets you go back) of the [`Bow\Http\Redirect`](https://bowphp.com/api/master/Bow/Http/Redirect.html) class.
