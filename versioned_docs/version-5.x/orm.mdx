...
:::info
An ORM (Object Relation Mapping) is a way of linking tables together using classes. Each record in a table represents an object that can be related to other records.
:::

The ORM included with BowPHP provides a simple and elegant ActiveRecord implementation for working with your database. Each database table has a corresponding "model" that is used to interact with that table. Models allow you to query data in your tables, as well as insert new records.

In any self-respecting framework, there is an ORM system with a friendly name. Bow's is called **Barry**.

:::warning Prerequisites
Before you begin, make sure you have configured a database connection in `config/database.php`.
:::
...
After the model is created, a file with the same name will be created — in our case `Todo.php` — at the root of the `app/Models` folder.
...
Note that we did not tell Barry which table to use for our `Todo` model. By convention, the plural "snake_case" name of the class will be used as the table name unless another name is explicitly specified.
...
class Todo extends Model
{
  ...
  protected string $table = 'todos'; // resolved as "app_todos"
}
```
...
You can also specify the **type** of the primary key and disable auto-increment (for example, for UUIDs or composite string identifiers):
...
You can also change the connection on a per-call basis:
...
The `retrieve` and `retrieveBy` methods can also be used to retrieve information:
...
To get the most recently created record according to the column declared in `$latest` (`created_at` by default), use the static `latest()` method:
...
Barry's `all` method will return all results in the model's array. Since each Barry model serves as a [query builder](./query-builder), you can also add constraints to queries, then use the `get` method to retrieve the results:
...
:::tip Going further
This section covers the most common Query Builder methods. For complete documentation with all the advanced options, see the [Query Builder](./query-builder) page.
:::

## Retrieving aggregates

You can also use the `count`, `sum`, `max`, and other aggregation methods provided by the [query builder](./query-builder). These methods return the appropriate scalar value instead of a full model instance:
...
You can also use the `update` method. However, you must set conditions to limit the impact of the update.
...
The `update` method expects an array of column-value pairs representing the columns to update.
...
Barry provides four types of relationships exposed via the `Bow\Database\Barry\Concerns\Relationship` trait (included by default in the `Model` class): `hasOne`, `hasMany`, `belongsTo`, and `belongsToMany`.

You declare a relationship as a method on the model that returns the relationship object. Accessing this method **as a property** triggers resolution and returns the results.
...
A post has multiple tags via a pivot table:
...
All methods accept two optional parameters to specify the join columns. The default names are derived from the name of the related table (`users` → `user_id`) and the model's primary key (`id`).

:::warning Argument order
`hasOne` and `hasMany` do **not** expect their two keys in the same order. Refer to the signatures below rather than your intuition.
:::
...
// hasMany(related, $local_key [this table], $foreign_key [related table])
...
- `$post->author()` — returns the relationship object (useful for adding constraints: `$post->author()->where('active', true)->first()`).
- `$post->author` — returns **the results** (instance / Collection).
:::

### Lazy loading

When you access a relationship **as a property**, Barry runs the query at that moment. The result is then kept in memory on the model instance: subsequent accesses return the **same** already-loaded object without re-running a query.

```php
$post = Post::retrieve(42);

$post->author;  // 1 query executed here
$post->author;  // no query: same instance returned
```
...
`eager()` works with all four relationship types (`hasOne`, `hasMany`, `belongsTo`, `belongsToMany`). Once preloaded, the relationship behaves exactly like property access, but without an additional query:
...
:::tip Keep in mind
- `eager()` chains onto the query and applies at the time of `get()`.
- If no related record exists, a "many" relationship returns an empty `Collection` and a "one" relationship returns `null`.
- The list of relationships to preload is reset after each `get()`: it does not "leak" into the next query on the same model.
:::
...
To exclude certain attributes during JSON serialization or conversion to an array:
...
{
  /**
   * The attribute type conversions.
   */
...
Global queries like `Todo::all()` or `Todo::where(...)->get()` return **all** rows, including soft-deleted ones. This is intentional: Bow does not apply an automatic global scope. Always use `Todo::withoutTrashed()` when you want to exclude archived rows.
:::

**5. Events**

The `model.deleting` / `model.deleted` hooks continue to fire on `delete()` (a soft delete is still a delete from a business standpoint). Four additional events are available:
...
Barry lets you intercept various operations on models via events:
