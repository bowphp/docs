...
Chaque méthode `add*` du tableau ci-dessus a une jumelle `change*` qui modifie
le type ou les attributs d'une colonne existante (`changeString`, `changeInteger`,
...
Pour les cas non couverts par les helpers, la méthode générique
`changeColumn($name, $type, $attr)` est disponible.
...
### Supprimer une contrainte (rollback)

Pour défaire les contraintes — utile dans la méthode `rollback()` :

```php
$this->alter('posts', function (Table $table) {
  $table->dropForeign('user_id');          // ou un tableau de colonnes
...
| Méthode | Description |
|---------|-------------|
| `dropForeign($name)` | Supprime une clé étrangère (chaîne ou tableau) |
| `dropIndex($name)` | Supprime un index |
| `dropUnique($name)` | Supprime une contrainte UNIQUE |
| `dropPrimary()` | Supprime la clé primaire |
