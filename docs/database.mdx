...
Ou via le helper `app_db`:

```php
$users = app_db('sqlite')->select(...);
```
...
Pour exécuter une requête brute de type `SELECT` nous devrez utiliser la méthode `Database::select` ou le helper `app_db_select`. On considère notre table `pets` et que nous sommes bien connectés à la base de donnée.
...
Notez que la valeur retournée par la méthode `app_db_select` est un `array` ou `null` s'il n'y a aucune informations.
...
Pour exécuter une requête brute de type `INSERT` nous devrez utiliser la méthode `Database::insert` ou le helper `app_db_insert`. On considère toujours notre table `pets` et que nous sommes bien connectés à la base de donnée.
...
Notez que la valeur retournée par la fonction `app_db_insert` est un `int` ou `number` qui est le nombre d'insertion.
...
Pour exécuter une requête brute de type `UPDATE` nous devrez utiliser la méthode `Database::update` ou le helper `app_db_update`. On considère toujours notre table `pets` et que nous sommes bien connectés à la base de donnée.
...
Pour exécuter une requête brute de type `DELETE` nous devrez utiliser la méthode `Database::delete` ou le helper `app_db_delete`. On considère toujours notre table `pets` et que nous sommes bien connectés à la base de donnée.
...
Pour exécuter une requête brute autre que `SELECT`, `UPDATE`, `INSERT`, `DELETE`. Il y a une méthode faite pour `Database::statement` ou le helper `app_db_statement`.
...
```php
$results = app_db_table('pets')->join('authors', 'authors.id', 'pets.author_id')->get();
```
