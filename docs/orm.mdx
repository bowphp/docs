...
// Avec findBy
$todo = Todo::retrieveBy('id', 1);
```

> La méthode peut aussi retourner `null` dans le cas où il n'y a aucun enrégistrement trouvé
...
Dans cet exemple, nous affectons le paramètre de nom de la requête HTTP entrante à les attributs `title`, `budget` de l'instance de modèle `App\Models\Todo`. Lorsque nous appelons la méthode `persist`, un enregistrement sera inséré dans la base de données. Les horodatages `created_at` et `updated_at` seront automatiquement définis lorsque la méthode de sauvegarde sera appelée, il n'est donc pas nécessaire de les définir manuellement.

### Insert via CREATE

Les objets Active Record peuvent être créés à partir d'un hachage, d'un bloc ou avoir leurs attributs définis manuellement après la création. La nouvelle méthode renverra un nouvel objet tandis que `create` renverra l'objet sans l'enregistrer dans la base de donnée. Vous devez ensuite lancer l'insert avec la méthode `persist`;
...
Ou bien vous pouvez aussi utiliser la methode `update`. Seulement vous dévez define les conditions pour ainsi limiter l'impact de la mise à jour.
