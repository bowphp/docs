...
rather than defining closures in the route files. Controllers
are stored in `app/Controllers/`.

Bow supports two declaration styles:

- **Routes defined in `routes/app.php`** — routes point to a
  controller via the `Controller::action` syntax (the style described in most
  of this document).
...
:::note PUT only (not PATCH)
The `update` method is registered as **PUT** only. If you also want
...
routes directly on the class with PHP 8 attributes:
...
You then only need to register the class in the route file:
...
The `name:` prefix from `#[Controller]` is concatenated **verbatim** to the name of
...
Customize the component namespaces in `app/Kernel.php`:
