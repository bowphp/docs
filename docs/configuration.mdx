...
All BowPHP configuration files are centralized in the `config/` folder. Each file is documented to make customization easier.
...
"default value" parameter. Passing a second argument writes the value into the
...
The second argument of `config()` is a **setter**: it writes the value into
...
| `session.php` | Sessions (driver, duration, cookies) |
