...
`get($key)` first looks in the merged routes/query/body and offers a default
fallback (`get('name', 'Invité')`). Reserve `query()` / `post()` for cases
...
*Bags* let you store values within a single request (for example permissions
resolved by a middleware, which a controller will read later) without polluting
the inputs.
