...
Dans ce cas nous avons `users` qui se répété plusieurs fois. Comment bien organiser tout ça ? La réponse est le préfixage de route. Alors la méthodes `prefix` nous permet de grouper plusieur URLs.
