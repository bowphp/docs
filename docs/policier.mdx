...
To install the package, you need to use `composer` (the PHP package manager) like this:
...
return [
  ...
  /**
   * Signature using your RSA, which loads automatically if the hashing key is of RSA type
   */
  "keychain" => [
    /**
...
:::info Token Instance
`$token` is an instance of `Policier\Token` and implements the magic method `__toString`. You can get the expiration time with `expireIn` and use `getToken` to retrieve the token value.
:::
...
`decode()` returns an instance of `Policier\Token`. Use its methods
`getHeaders()` / `getHeader($name)` / `getClaim($name)` / `getClaims()` to
...
subject (the same `$id` you passed to `encode()`). As its second argument, the
`validate` method takes that identifier — **not** an array of claims.
...
If you also want to check application-level claims (name, role, etc.),
...
If you are using [BowPHP](https://github.com/bowphp/app), you can use the configuration plugin `Policier\Bow\PolicierConfiguration::class` and the middleware `Policier\Bow\PolicierMiddleware::class`.
...
To publish the custom middleware and override Policier's default one, simply add the middleware to the `app/Kernel.php` file with the `policier` key.
:::
