...
Seeding lets you populate your database with test data for development. Each seeder is associated with a table and generates data using the Faker library.
...
A file will be created in the `seeders/` folder with a timestamp:
...
The `depends()` method lets you define the seeders that must be run before this one:
...
class Posts20260309130000
{
  public function run()
  {
    ...
    // Retrieve the IDs of users that have already been seeded
...
:::tip Localization
Use `FakerFactory::create('fr_FR')` for French data.
:::
