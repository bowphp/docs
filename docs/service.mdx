...
Let's create a complete service to manage users:
...
reserved for the framework — **do not override it** (you would bypass the
loading of the other configurations).
...
:::tip Recommendations
- **One service = one responsibility**: Each service should have a single domain of responsibility.
...
- **Naming**: Use explicit names such as `UserService`, `PaymentService`, `NotificationService`.
- **Tests**: Services make it possible to isolate business logic and test it independently.
:::
