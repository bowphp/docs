...
6. **Middleware**: Middlewares perform specific tasks, such as checking authentication or validating data.
7. **Controller**: Once the route is validated, the controller runs the business logic (for example, retrieving data or performing an action).
...
routes directly on the controller class without a centralized routes
file:
...
See [Routing > Attribute-based routing](./routing) for details on the
...
independent, has its own configuration, and serves a specific role.
...
- `run()` — starts the module (useful for eager resources)
